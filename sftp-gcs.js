...
const argv = require("yargs")
  .usage(
    "$0 --bucket BUCKET_NAME [--port PORT_NUM] [--user USER_NAME] [--password PASSWORD]"
  )
  .options("bucket")
  .describe("bucket", "GCS bucket to work with")
  .string("bucket")
  .nargs("bucket", 1)
  .demandOption("bucket", "Must supply a GCS bucket")
  .options("port")
  .describe("port", "Port number to listen upon")
  .number("port")
  .nargs("port", 1)
  .default("port", 22, "Default port is 22")
  .options("user")
  .describe("user", "Userid for SFTP client")
  .string("user")
  .nargs("user", 1)
  .default("user", "")
  .options("password")
  .describe("password", "Password for SFTP client")
  .string("password")
  .nargs("password", 1)
  .default("password", "")
  .options("public-key-file")
  .describe("public-key-file", "Publish SSH key")
  .string("public-key-file")
  .nargs("public-key-file", 1)
  .default("public-key-file", "")
  .options("service-account-key-file")
  .describe("service-account-key-file", "Key file for service account")
  .string("service-account-key-file")
  .nargs("service-account-key-file", 1)
  .default("service-account-key-file", "")
  .options("debug")
  .describe("debug", "Set the debugging log level")
  .string("debug")
  .nargs("debug", 1)
  .default("info", "").argv;
...
function getHostKey() {
  if (fs.existsSync("/etc/ssh/ssh_host_rsa_key")) {
    try {
      ...
      logger.warn(
        `Unable to read /etc/ssh/ssh_host_rsa_key even though it exists.`
      );
    }
  }
  logger.warn(
...
new ssh2.Server(
  ...
  function (client) {
    ...
    client.on("authentication", function (ctx) {
      ...
      switch (ctx.method) {
        ...
        case "publickey":
          logger.debug(
            `key: ${util.inspect(ctx.key)}, signature: ${util.inspect(
              ctx.signature
            )}`
          );
      ...
      client.on("session", function (accept, reject) {
        ...
        session.on("sftp", function (accept, reject) {
          ...
          sftpStream.on("OPEN", async function (reqId, filename, flags, attrs) {
            logger.debug(
              `OPEN<${reqId}>: filename: "${filename}", flags: ${SFTPStream.flagsToString(
                flags
              )}`
            );
            ...
            if (flags & SFTPStream.OPEN_MODE.WRITE) {
              ...
              fileRecord.writeStream.on("error", (err) => {
                logger.debug(
                  `Detected an error with writeStream to the GCS file: ${err}`
                );
          ...
          sftpStream.on("WRITE", function (reqId, handleBuffer, offset, data) {
            ...
            if (fileRecord.gcsError === true) {
              logger.debug(
                `Returning failure in WRITE because of flagged gcsError`
              );
              return sftpStream.status(reqId, STATUS_CODE.FAILURE);
            }
          ...
          sftpStream.on(
            "READ",
            async function (reqId, handleBuffer, offset, requestedLength) {
              ...
              try {
                const data = await fileRecord.getGCSData(
                  offset,
                  requestedLength
                );
                if (data === null) {
                  // If we get null, we have reached the end of file.
          ...
          sftpStream.on("MKDIR", async function (reqId, path, attrs) {
            ...
            logger.debug(
              `MKDIR<${reqId}>: path: "${path}", attrs: ${util.inspect(attrs)}`
            );
            try {
              path = normalizePath(path);
              const dirName = path + "/";
          ...
          sftpStream.on("OPENDIR", async function (reqId, path) {
            ...
            if (path !== "") {
              ...
              try {
                ...
                if (fileList.length == 0) {
                  logger.debug(
                    `we found no files/directories with directory: "${path}"`
                  );
          ...
          sftpStream.on("READDIR", async function (reqId, handleBuffer) {
            ...
            logger.debug(
              `READDIR<${reqId}>: handle: ${fileRecord.handle}, path: "${fileRecord.path}"`
            );
          ...
          sftpStream.on("FSTAT", async function (reqId, handleBuffer) {
            ...
            logger.debug(
              `FSTAT<${reqId}>: handle: ${fileRecord.handle} => path: "${fileRecord.path}"`
            );
            if (!fileRecord.path) {
          ...
          sftpStream.on("SETSTAT", function (reqId, path, attrs) {
            ...
            logger.debug(
              `SETSTAT<${reqId}>: path: "${path}", attrs: ${util.inspect(
                attrs
              )}`
            );
          ...
          sftpStream.on("RENAME", async function (reqId, oldPath, newPath) {
            ...
            logger.debug(
              `RENAME<${reqId}>: oldPath: ${oldPath}, newPath: ${newPath}`
            );
          ...
          sftpStream.on("CLOSE", function (reqId, handleBuffer) {
            ...
            if (fileRecord.writeStream) {
              ...
              if (fileRecord.gcsError) {
                logger.debug(
                  `Returning error because of previous GCS Error with write stream`
                );
                return sftpStream.status(reqId, STATUS_CODE.FAILURE);
              }
  ...
  .listen(SERVER_PORT, "0.0.0.0", function () {
    ...
    logger.info(
      `Public key file: ${publicKeyFile === "" ? "Not set" : publicKeyFile}`
    );
    logger.info(
      `Service account key file: ${
        serviceAccountKeyFile === "" ? "Not set" : serviceAccountKeyFile
      }`
    );
  })
  .on("error", (err) => {
    ...
    if (err.code === "EACCES") {
      logger.info(
        `It is likely that an application is already listening on port ${err.port}.`
      );
    }
  });
