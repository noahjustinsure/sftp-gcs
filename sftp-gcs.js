...
const loggingWinston = new LoggingWinston({ logName: 'sftp-gcs' })
...
const MODE_FILE =
	fs.constants.S_IFREG | fs.constants.S_IRWXU | fs.constants.S_IRWXG | fs.constants.S_IRWXO
const MODE_DIR =
	fs.constants.S_IFDIR | fs.constants.S_IRWXU | fs.constants.S_IRWXG | fs.constants.S_IRWXO
...
function fileLongEntry(name, isDirectory, size, padding, created) {
	...
	return `${isDirectory ? 'd' : '-'}rw-rw-rw- 1 none none ${String(size).padStart(
		padding
	)} ${created} ${name}`
} // fileLongEntry
	...
	function (client) {
		...
		client.on('authentication', function (ctx) {
			...
			switch (ctx.method) {
				...
				case 'publickey':
					logger.debug(
						`key: ${util.inspect(ctx.key)}, signature: ${util.inspect(ctx.signature)}`
					)
		...
		client.on('ready', function () {
			...
			client.on('session', function (accept, _reject) {
				...
				session.on('sftp', function (accept, _reject) {
					...
					// o EXCL
					//
					sftpStream.on('OPEN', async function (reqId, filename, flags, _attrs) {
						logger.debug(
					...
					sftpStream.on('READDIR', async function (reqId, handleBuffer) {
						...
						bucket.getFiles(
							...
							(err, fileList, _nextQuery, _apiResponse) => {
